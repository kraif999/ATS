...
#shinyApp(ui = ui, server = server, options = list(port = 7777, host = "127.0.0.1"))

################################################################################################
# next step (wrap to .exe)
################################################################################################


# Run app on a random free port and write port to file
app <- shinyApp(ui = ui, server = server)

# Let Shiny pick a free port
shinyPort <- httpuv::randomPort()

# Write the port to a file for Electron to read
writeLines(as.character(shinyPort), "port.txt")

# Run the app
runApp(app, host = "127.0.0.1", port = shinyPort)

################################################################################################
#executablePackeR::pack(app_name = "myapp", electron_settings = list(), option = list())
# 
# if (interactive()) {
# executablePackeR::pack(
#   app_name = "myapp",
#   electron_settings = list(
#     c("product_name_template", "Nero"),
#     c("app_description_template", "Test ideas"),
#     c("author_name_template", "Kraif999")
#   ),
#   option = list(start_timeout = 20000)  # optional
# )
# }