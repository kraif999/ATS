...
apply_rm, flat_after_event, dynamic_limits, max_risk, reward_ratio, run_via_cpp) {
  ...
  annual_vol_mult <- ifelse(meta$assets[[symbol]]$class %in% "Cryptocurrency", sqrt(365), sqrt(252))

  # Calculate annualized volatility, cumulative PnLs, and return of portfolio
  self$data <- self$data %>%
    mutate(
      annual_vol = rollapply(value, width = 30, FUN = sd, fill = NA, align = "right") * annual_vol_mult,
...
estimate_trading_profile = function(data_subset, strategy_type, symbol) {
  ...
  profit_scale <- ifelse(meta$assets[[symbol]]$class %in% "Cryptocurrency", 365, 252)
