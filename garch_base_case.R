...
vxData <- readRDS("vxDataAll.rds") # loading VIX futures monthly and weekly contracts data from CBOE website
#table(vxData$Type)

vxData2 <- vxData %>%
  filter(Type == "monthly") %>%
    select(TradeDate, ContractName, Close, Type, ttm)
...
# Different realized volatility estimators for returns, see TTR library
histVolest <- merge(
  garman <- as.xts(na.omit(volatility(ohlc, calc = "garman"))) / sqrt(252),
  close <- na.omit(volatility(ohlc[,4], calc = "close")) / sqrt(252),
  parkinson <- as.xts(na.omit(volatility(ohlc, calc = "parkinson"))) / sqrt(252)
) %>% 
  as.data.frame %>% 
    rename_with(~ c("garman", "close", "parkinson")) %>%
      mutate(TradeDate = as.Date(rownames(.))) 
...
histVolest %>% 
  ...
  geom_line(aes(x = TradeDate, y = close, color = "Close-to-Close")) +
  geom_line(aes(x = TradeDate, y = garman, color = "Garman-Klass")) +
  geom_line(aes(x = TradeDate, y = parkinson, col = "Parkinson")) + 
  scale_color_manual(values = c(
    'Close-to-Close' = 'green',
    'Garman-Klass'='black',
    'Parkinson'='darkblue')) + 
  labs(color = 'Historical volatility estimators')

# Check how many times Close-to-Close exceeds Garman and Parkinson (each)
histVolest$compare <- 0
for (i in 1:nrow(histVolest)) {
  histVolest$compare[i] <- ifelse(histVolest$close[i] > histVolest$garman[i] && histVolest$close[i] > histVolest$parkinson[i], 1, 0)
}

print(HVclose_other <- sum(histVolest$compare) / nrow(histVolest) * 100) # 75% jointly "Close-to-Close" is higher than the other two

# Base GARCH case with 'base case' parameters configuration, also see 'rugarch' library
...
# Base case strategy assumptions
dfl <- 0.25 # financial leverage
m <- 1000 # the multiplicator for one VIX futures point change is 1000 USD
capital <- 1000000 # initial investment equals 1 USDm

# Final data for plot and table are stored here:
colp <- c(
  ...
  "% of long signal to all signals", 
  "RMSE", "Corrret sign change prediction")

resGARCHbase <- data.frame(matrix(NA, nrow = dim(listgarch)[1], ncol = 8)) # number of parameters to be varied
colnames(resGARCHbase) <- colp

equityline <- data.frame(matrix(NA, nrow = 1700, ncol = dim(listgarch[1])))
colnames(equityline) <- c("sGARCH")
...
# Realized volatility estimation by different approaches, in GARCH base case 'Close' approach is used:
roll.df$RV <- 0
ohlc <- sp500gk2 %>% select(GSPC.Open, GSPC.High, GSPC.Low, GSPC.Close)
colnames(ohlc) <- c("Open", "High", "Low", "Close")

roll.df$RV <- as.numeric(volatility(ohlc[,4], n = 21, calc = "close", N = 252) / sqrt(252))
roll.df <- roll.df %>% na.omit
...
tmp3$eqlGARCH <- 0 # capital + PnL * nop * m - TC * delta number of positions (nop)
...
# additional check for futures position close out: selling the nearest to expiration at ttm1 = 1, buying the second close to expiration futures
# tmp3$eqlVIXBH2 <- 0
# tmp3$eqlVIXBH2[1] <- capital 
# tmp3$nopBH[1] <- capital * dfl / (tmp3$Close1 * m) # number of positions at first entry

# for(p in 2:nrow(tmp3)) {
#   TC <- ifelse(tmp3$ttm1[p] == 0, 2 * abs(tmp3$nopBH[p-1]), 0) # adding transactional costs given contract rolling forward
#   tmp3$eqlVIXBH2[p] <- ifelse(tmp3$ttm1[p] == 1,
#     tmp3$eqlVIXBH2[p-1] - ((tmp3$Close1[p] - tmp3$Close1[p-1]) * tmp3$nopBH[p-1] * m) - TC * floor(tmp3$eqlVIXBH[p] * dfl / (tmp3$Close[s] * m),
#     tmp3$eqlVIXBH2[p-1] + (tmp3$Close[p] - tmp3$Close[p-1]) * tmp3$nopBH[p-1] * m - TC)
#   tmp3$nopBH[p] <- floor(tmp3$eqlVIXBH2[p] * dfl / (tmp3$Close[p] * m)) # using 'floor' function to round 
# }
# tmp3$r_eqlVIXBH2 <- quantmod::Delt(tmp3$eqlVIXBH2) # equity line return
# end check

# Performance metrics of strategy based on GARCH
listgarch$aR <- round(as.numeric(Return.annualized(as.numeric(tmp3$r_eqlGARCH), scale = 252, geometric = TRUE) * 100), 3)
listgarch$aSD <- round(as.numeric(StdDev.annualized(as.numeric(tmp3$r_eqlGARCH), scale = 252) * 100), 3)
listgarch$IR <- round(as.numeric(listgarch$aR / listgarch$aSD), 3) 
listgarch$MD <- round(as.numeric(maxDrawdown(as.numeric(tmp3$r_eqlGARCH), weights = NULL, geometric = TRUE, invert = TRUE) * 100),3)
listgarch$trades <- sum(tmp3$trades) # count of all trades
listgarch$signal_long <- round(long/(long+short),3) * 100 # long signal percentage
listgarch$rmse <- rmse
listgarch$elf <- ELF

# Performance metrics of passive B&H strategy (investing in VIX futures)
colnames(listgarch)[7:dim(listgarch)[2]] <- colp
...
save(passive, file = "PassiveStrategiesMetrics.RData")
rm(passive1, passive2)

# GARCH coefficients binding check (alpha + beta < 1)
garch_coefficients <- function(roll, plot = TRUE) {
  roll_coeff <- roll@model[["coef"]]
  roll_coeff_matrix <- matrix(NA, nrow = length(roll_coeff), ncol = 6)
  colnames(roll_coeff_matrix) <- c("mu", "ar1", "ma1", "omega", "alpha1", "beta")
  
  for (i in 1:length(roll_coeff)) {
    roll_coeff_matrix[i, ] <- unlist(lapply(roll_coeff[[i]]$coef[1:6, 1], function(x) x))
  }
  roll_coeff_matrix <- as.data.frame(roll_coeff_matrix)
  
  refit_dates <- matrix(NA, length(roll_coeff), 1)
  for(i in 1:length(roll_coeff)) {
    refit_dates[i] <- as.character(roll_coeff[[i]]$index[1])
  }
  colnames(refit_dates)[1] <- "RefitDate"
  
  coefs <- roll_coeff_matrix %>% cbind(refit_dates) %>% mutate(RefitDate = as.Date(RefitDate))
  coefs$ab <- coefs$alpha1 + coefs$beta
  
  if (plot) {
    coefs %>%
      ggplot() +
      ylim(0,1) + 
      ggtitle("The sum of alpha1 and beta coefficients for sGARCH across quarterly refits (21) with bands") +
      ylab("Coefficient sum value") +
      geom_line(aes(x = RefitDate, y = alpha1 + beta, color = "alpha1 + beta")) + 
      geom_hline(yintercept = 1, linetype = "dashed", color = "red") + 
      scale_color_manual(values = c(
        'alpha1 + beta' = 'darkblue')) +
...
garch_coefficients(roll)

vix_with_buy_and_hold <- tmp3 %>% select(TradeDate,ContractName1, Close1, ttm1, ContractName2, Close2, ttm2, eqlSP500BH, eqlVIXBH, eqlGARCH)
...
save(vix_with_buy_and_hold, file = "PassiveStrategiesEquityLine.RData")
